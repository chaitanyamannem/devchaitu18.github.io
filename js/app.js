...
var app = angular.module('syncBudget',['ngRoute','ui.bootstrap','ngTouch','ngAnimate']);
	...
	app.directive('tree', function(){
		...
		return {
			...
			template: "<ul><leaf ng-repeat='leaf in tree' leaf='leaf'><span>{{leaf.categoryName}}</span></leaf></ul>"
		}
		...
		app.controller('showCategoriesController', function($scope,$log){
			...
			$scope.getCategories = function(){
				...
				for(var i = 0; i < $scope.primarycategories.length; i++){
					var primaryCategoryName = $scope.primarycategories[i].categoryName = $scope.primarycategories[i].get('name');
