(function () {
    var app = angular.module('syncBudget', ['ngRoute', 'ui.bootstrap', 'ngTouch', 'ngAnimate', 'firebase']);
    ...
    app.controller('showCategoriesController', function ($scope, $log) {
        ...
        $scope.getCategories = function () {
            ...
            $scope.primarycategories = categoriesTable.query({
                type: 'Primary'
            });
            for (var i = 0; i < $scope.primarycategories.length; i++) {
                ...
                $scope.primarycategories[i].nodes = categoriesTable.query({
                    type: 'Secondary',
                    primary: primaryCategoryName
                });
                for (var j = 0; j < $scope.primarycategories[i].nodes.length; j++) {
                    ...
                    $scope.primarycategories[i].nodes[j].nodes = categoriesTable.query({
                        type: 'Tertiary',
    ...
    app.controller('EditExpenseModalController', function ($scope, $modalInstance) {
        ...
        $("#edit_expense_tag_handler").tagHandler({
            ...
            onAdd: function (tag) {
                $scope.thisExpenseTags.push(tag);
                $scope.$apply();
            },
            onDelete: function (tag) {
                $scope.thisExpenseTags.pop(tag);
                $scope.$apply();
            },
            autocomplete: true
        });
    ...
    app.controller('showGraphsController', function ($scope) {
        ...
        var expensesForMonth = function (queryMonth, queryYear) {
            ...
            return $scope.datastore.getTable('expenses').query({
                month: queryMonth,
                year: queryYear
            });
        }
